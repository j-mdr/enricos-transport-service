...
export const heroBgImage = defineType({
  ...
  fields: [
    ...
    defineField({ name: "image", title: "Achtergrond afbeelding", type: "imageWithAlt", validation: (Rule) => Rule.required() }),
  ],
  preview: { select: { title: "title" }, prepare: ({ title }) => ({ title, subtitle: "HeroBgImageSection" }) },
});
