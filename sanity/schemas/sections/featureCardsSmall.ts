...
export const featureCardsSmall = defineType({
  ...
  fields: [
    ...
    defineField({ name: "features", title: "Feature kaarten", type: "array", of: [{ type: "featureCardSmall" }], validation: (Rule) => Rule.required().min(1) }),
  ],
  preview: { select: { title: "title" }, prepare: ({ title }) => ({ title, subtitle: "FeatureCardsSmallSection" }) },
});
