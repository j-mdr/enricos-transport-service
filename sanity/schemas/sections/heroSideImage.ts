...
export const heroSideImage = defineType({
  ...
  fields: [
    ...
    defineField({ name: "image", title: "Afbeelding", type: "imageWithAlt", validation: (Rule) => Rule.required() }),
  ],
  preview: { select: { title: "title" }, prepare: ({ title }) => ({ title, subtitle: "HeroSideImageSection" }) },
});
