...
export const faqAccordions = defineType({
  ...
  fields: [
    ...
    defineField({ name: "faqs", title: "FAQ items", type: "array", of: [{ type: "faqItem" }], validation: (Rule) => Rule.required().min(1) }),
  ],
  preview: { select: { title: "title" }, prepare: ({ title }) => ({ title, subtitle: "FaqAccordionsSection" }) },
});
