...
export const teamMemberCards = defineType({
  ...
  fields: [
    ...
    defineField({ name: "teamMembers", title: "Teamleden", type: "array", of: [{ type: "profileCard" }], validation: (Rule) => Rule.required().min(1) }),
  ],
  preview: { select: { title: "title" }, prepare: ({ title }) => ({ title, subtitle: "TeamMemberCardsSection" }) },
});
